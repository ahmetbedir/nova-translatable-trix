...
export default {
  ...
  methods: {
    ...
    fill(formData) {
      ...
      formData.append(this.fieldAttribute + 'DraftId', this.draftId)
    },
    ...
    uploadAttachment(attachment) {
      ...
      Nova.request()
        .post(
          `/nova-api/${this.resourceName}/trix-attachment/${this.fieldAttribute}`,
          data,
          {
    ...
    cleanUp() {
      if (this.field.withFiles) {
        Nova.request()
          .delete(
            `/nova-api/${this.resourceName}/trix-attachment/${this.fieldAttribute}/${this.draftId}`
          )
          .then(response => {})
          .catch(error => {})
      }
  ...
  computed: {
    fieldAttribute(){
        if(this.field.hasOwnProperty('translatable')) {
            return this.field.translatable.original_attribute;
        }
